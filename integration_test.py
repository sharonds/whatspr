...
def main():
    ...
    if all(results):
        ...
        failed_prs = []
        if not results[0]: failed_prs.append("PR1 (Atomic Tools)")
        if not results[1]: failed_prs.append("PR2 (Goal-Oriented Prompt)")
        if not results[2]: failed_prs.append("PR3 (File Search)")
