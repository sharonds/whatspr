...
async def test_timeout_improvements():
    ...
    try:
        reply, thread_id, tool_calls = await run_thread_with_retry(
            None, "I want to announce a product launch for my AI startup", timeout_seconds=12.0
        )
...
async def test_thread_state_management():
    ...
    webhook_data = {'From': phone, 'Body': 'Hello', 'ProfileName': 'Test User'}
    ...
    try:
        ...
        await agent_hook(mock_request2)
        ...
        await agent_hook(mock_request3)
...
async def test_retry_logic():
    ...
    with patch('app.agent_endpoint.run_thread', side_effect=mock_failing_run_thread):
        try:
            reply, thread_id, tool_calls = await run_thread_with_retry(
                None, "Test retry logic", timeout_seconds=10.0
            )
...
async def test_reliability_improvements():
    ...
    colored_print(
        f"  📊 Success rate: {success_count}/{total_tests} ({success_rate:.1f}%)", "yellow"
    )
...
async def test_concurrent_sessions():
    ...
    async def run_concurrent_session(session_id):
        try:
            ...
            webhook_data = {'From': phone, 'Body': 'reset', 'ProfileName': f'User {session_id}'}
