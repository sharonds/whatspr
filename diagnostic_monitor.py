...
class WhatsAppDiagnostics:
    ...
    def test_whatsapp_endpoint(self, message_body="We raised 250K from Elon Musk"):
        ...
        try:
            ...
            print(f"{status_icon} {elapsed:.2f}s | 📊 {response.status_code} | 📝 {len(response.text)} chars | {result_type}")
            ...
            return {'success': False, 'elapsed': elapsed, 'result_type': 'timeout', 'message': message_body}
        except Exception as e:
            elapsed = time.time() - start_time
            print(f"❌ ERROR after {elapsed:.2f}s: {e}")
            return {'success': False, 'elapsed': elapsed, 'result_type': 'error', 'message': message_body, 'error': str(e)}
    ...
    def check_system_resources(self):
        ...
        try:
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cpu_percent', 'memory_percent']):
                try:
                    if 'uvicorn' in proc.info['name'] or 'python' in proc.info['name']:
                        print(f"🔍 Process {proc.info['name']} (PID {proc.info['pid']}): CPU {proc.info['cpu_percent']:.1f}%, Memory {proc.info['memory_percent']:.1f}%")
            ...
            try:
                ...
                result = subprocess.run(['top', '-l', '1'], capture_output=True, text=True, timeout=5)
    ...
    def run_comprehensive_test(self):
        ...
        complex_result = self.test_whatsapp_endpoint("We are TechCorp and we just raised $15M Series A from Venture Partners. Our CEO Jane Smith said this will accelerate our mission.")
    ...
    def analyze_results(self, openai_success, openai_time):
        ...
        if not openai_success:
            print("🚨 CRITICAL: OpenAI API is NOT responding - This is likely the primary bottleneck")
        elif openai_time > 5:
        ...
        print(f"   tail -50 server_diagnostic.log | grep -E '(error|exception|failed)'")
        print(f"   tail -100 server_diagnostic.log | grep -E '(ai_timeout|ai_error)'")
        print(f"   curl -s https://status.openai.com/api/v2/status.json | python3 -m json.tool")
