...
def main():
    ...
    try:
        ...
        message = client.beta.threads.messages.create(
            thread_id=thread_id, role="user", content=test_message
        )
        ...
        run = client.beta.threads.runs.create(
            thread_id=thread_id, assistant_id=assistant_id, timeout=10
        )
            ...
            if assistant_messages:
                ...
                if latest_response.content and len(latest_response.content) > 0:
                    ...
                    if hasattr(content, 'text') and hasattr(content.text, 'value'):
                        ...
                        print(
                            f"❌ No text content in response (content type: {type(content).__name__})"
                        )
                else:
        ...
        message = client.beta.threads.messages.create(
            thread_id=complex_thread_id, role="user", content=complex_message
        )

        complex_run = client.beta.threads.runs.create(
            thread_id=complex_thread_id, assistant_id=assistant_id, timeout=10
        )
        ...
        while True:
            ...
            try:
                complex_run = client.beta.threads.runs.retrieve(
                    thread_id=complex_thread_id, run_id=complex_run.id
                )

                print(
                    f"   Complex check {check_count}: {complex_run.status} (elapsed: {elapsed:.1f}s)"
                )
                ...
                if complex_run.status == "completed":
                    ...
                    if complex_run.last_error:
                        ...
                        print(
                            f"   Error message: {getattr(complex_run.last_error, 'message', 'N/A')}"
                        )
                    break
