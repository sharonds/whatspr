...
class TestMVPSessionLifecycle(RateLimitedTestCase):
    ...
    def test_concurrent_mvp_users_basic(self):
        ...
        for phone in users:
            ...
            ), f"User {phone} had inconsistent thread_ids: {thread_ids}"
            ...
            for other_phone in users:
                if other_phone != phone:
                    ...
                    assert (
                        thread_ids[0] not in other_thread_ids
                    ), f"Thread collision between {phone} and {other_phone}"
    ...
    def test_session_ttl_expiration(self):
        ...
        short_ttl_config = SessionConfig(ttl_seconds=1, cleanup_interval=0.5, allow_test_values=True)
    ...
    def test_cleanup_during_active_conversations(self):
        ...
        cleanup_config = SessionConfig(ttl_seconds=2, cleanup_interval=0.5, allow_test_values=True)
    ...
    def test_session_isolation_stress(self):
        ...
        for i in range(user_count):
            ...
            assert all(
                result == expected_thread for result in user_results
