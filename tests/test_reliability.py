...
class TestOpenAIReliability:
    ...
    def test_assistant_exists_and_accessible(self):
        ...
        try:
            ...
            if assistant.tools:
                tool_names = [t.function.name if hasattr(t, 'function') else t.type 
    ...
    def test_thread_creation_reliability(self):
        ...
        log.info("thread_creation_stats", 
    ...
    def test_single_conversation_reliability(self):
        """Test a single conversation flow 10 times to measure failure rate."""
        bot = WhatsSim()
        results = []
        ...
        for attempt in range(10):
            log.info(f"conversation_attempt_{attempt}", start=True)
            ...
            try:
                for msg_idx, message in enumerate(test_messages):
                    ...
                    time.sleep(1)  # Realistic typing delay
            ...
            log.info(f"conversation_attempt_{attempt}", 
                    success=conversation_success,
                    errors=errors if not conversation_success else None)
            
            time.sleep(2)  # Cool down between attempts
        
        # Calculate statistics
        success_count = sum(1 for r in results if r["success"])
        failure_rate = (10 - success_count) / 10 * 100
        
        log.info("conversation_reliability_stats",
        ...
        assert success_count >= 5, f"Conversation reliability too low: {success_count}/10 ({failure_rate}% failure rate)"
    
    def test_concurrent_sessions(self):
        """Test multiple concurrent user sessions."""
        def run_session(phone_suffix: int) -> Dict:
            bot = WhatsSim(phone=f"+1555123{4560 + phone_suffix}")
            errors = []
            success = True
            ...
            try:
                ...
                responses.append(bot.send("TestCorp" + str(phone_suffix)))
        ...
        success_count = sum(1 for r in results if r["success"])
        log.info("concurrent_session_stats",
                success_rate=f"{success_count}/5",
                failed_sessions=[r["phone_suffix"] for r in results if not r["success"]])
        
        assert success_count >= 3, f"Concurrent session reliability too low: {success_count}/5"
    
    def test_tool_calling_reliability(self):
        """Test direct tool calling through run_thread."""
        successes = 0
        tool_call_counts = []
        errors = []
        ...
        for i in range(5):
            try:
                ...
                if len(tool_calls) > 0:
                    successes += 1
                    log.info(f"tool_calling_success_{i}", 
        ...
        log.info("tool_calling_stats",
                success_rate=f"{successes}/5",
                avg_tool_calls=avg_tool_calls,
                all_counts=tool_call_counts,
                errors=errors[:2] if errors else None)
        
        assert successes >= 3, f"Tool calling reliability too low: {successes}/5"
        assert avg_tool_calls > 0, "No tools being called on average"
...
class TestTimeoutAndRecovery:
    ...
    def test_response_time_distribution(self):
        """Measure response times to identify timeout issues."""
        bot = WhatsSim()
        response_times = []
        ...
        for i in range(5):
            ...
            try:
                ...
                log.info(f"response_time_{i}", seconds=elapsed, success=True)
            except Exception as e:
                elapsed = time.time() - start
                response_times.append(elapsed)
                log.error(f"response_timeout_{i}", seconds=elapsed, error=str(e))
        ...
        log.info("response_time_stats",
                avg_seconds=avg_time,
                max_seconds=max_time,
                all_times=response_times)
        
        assert max_time < 30, f"Response time too high: {max_time}s"
        assert avg_time < 10, f"Average response time too high: {avg_time}s"
    
    def test_session_recovery_after_error(self):
        """Test if sessions can recover after an error."""
        bot = WhatsSim()
        ...
        recovery_success = (
            response2 and "oops" not in response2.lower() and
            response3 and "oops" not in response3.lower()
        )
        
        log.info("session_recovery_test",
                long_message_response=response1[:100] if response1 else None,
                recovery_success=recovery_success)
        
        assert recovery_success, "Session failed to recover after error"
    ...
    def test_generate_failure_report(self):
        ...
        try:
            ...
            if r1 and r2 and "oops" not in r1.lower() and "oops" not in r2.lower():
                print(f"✅ Full conversation: SUCCESS")
            else:
                print(f"⚠️  Full conversation: PARTIAL (responses received but may contain errors)")
        except Exception as e:
