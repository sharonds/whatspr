...
class TestAPIFailureRecovery(RateLimitedTestCase):
    ...
    def test_openai_timeout_preserves_session(self):
        ...
        with patch('app.agent_runtime.get_client') as mock_client:
            mock_client.return_value.beta.threads.runs.create.side_effect = \
                asyncio.TimeoutError("OpenAI API timeout")
            
            # Send message that will trigger timeout
            response = self.client.post("/agent", data={
                "From": self.test_phone,
                "Body": "Tell me about funding announcements"
            })
            
            # Should return timeout fallback message
            assert response.status_code == 200
            response_text = response.text
            assert "processing your request" in response_text.lower() or \
                   "give me a moment" in response_text.lower()
    ...
    def test_concurrent_api_failures_isolation(self):
        ...
        with patch('app.agent_runtime.get_client') as mock_client:
            ...
            response1 = self.client.post("/agent", data={
                "From": phone1,
                "Body": "Message from phone1"
            })
            assert response1.status_code == 200
            
            # Phone2 and phone3 should succeed
            response2 = self.client.post("/agent", data={
                "From": phone2,
                "Body": "Message from phone2"
            })
            assert response2.status_code == 200
            
            response3 = self.client.post("/agent", data={
                "From": phone3,
                "Body": "Message from phone3"
            })
    ...
    def test_session_cleanup_during_api_failure(self):
        ...
        with patch('app.agent_runtime.get_client') as mock_client:
            mock_client.return_value.beta.threads.runs.create.side_effect = \
                Exception("API failure during cleanup test")
