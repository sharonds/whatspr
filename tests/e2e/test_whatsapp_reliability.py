...
class TestWhatsAppReliability:
    ...
    def test_production_failure_scenario(self):
        ...
        meaningful_indicators = [
            "what type", "announcement", "more information", "tell me", "funding", 
            "raised", "250k", "elon", "congratulations", "exciting", "great",
            "processing your request", "give me a moment", "try again"  # Timeout fallbacks
        ]
        found_indicators = [ind for ind in meaningful_indicators if ind in response_lower]
        assert len(found_indicators) > 0, f"Response should be meaningful. Response: {response.text}"
    ...
    def test_timeout_resistance(self):
        ...
        for i, message in enumerate(test_messages):
            ...
            assert response.status_code == 200, f"Message {i+1} failed with status {response.status_code}"
        ...
        def send_message_worker(message_id):
            ...
            try:
                ...
                return {
                    ...
                    'has_error': 'oops' in response.text.lower() or 'error' in response.text.lower(),
    ...
    def test_session_persistence_reliability(self):
        ...
        response4, _ = self._send_whatsapp_message("What information do you have so far?", phone=phone)
        ...
        assert len(found_context) >= 2, f"Should remember context. Found: {found_context}. Response: {response4.text}"
    ...
    def test_response_consistency(self):
        ...
        print(f"   Duration: avg={avg_duration:.2f}s, range={min(durations):.2f}-{max(durations):.2f}s")
        ...
        for response in responses:
            ...
            assert duration_diff < 5.0, f"Response time too inconsistent: {duration_diff:.2f}s from average"
            
            length_diff = abs(response['length'] - avg_length)
            assert length_diff < avg_length * 0.5, f"Response length too inconsistent: {length_diff} chars from average"
...
class TestWhatsAppEdgeCases:
    ...
    def test_twilio_webhook_format_compliance(self):
        ...
        assert response.status_code == 200, f"Expected 200, got {response.status_code}"
        assert response.headers.get("content-type", "").startswith("text/"), "Should return text content"
        ...
        print(f"✅ Twilio webhook compliance verified")
    ...
    def test_url_encoded_special_characters(self):
        ...
        for encoded_message, description in test_cases:
            ...
            response_lower = response.text.lower()
            assert "%20" not in response_lower, f"{description}: Response contains encoding artifacts"
            assert "%3A" not in response_lower, f"{description}: Response contains encoding artifacts"
